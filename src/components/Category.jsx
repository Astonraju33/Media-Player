...
function Category({removeCategoryVideoResponse}) {
  ...
  const videoDropped = async (e,categoryId)=>{
    ...
    console.log(data);
    //get category details wherewe have add video
...
  }
  return (
    ...
    <button onClick={handleShow} style={{width:'60px', height:'60px'}}  className='btn btn-danger rounded-circle fs-6'>+</button>
</div>
...
((item,index)=>(
  <div droppable="true" onDragOver={(e)=>dragOverCategory(e)} onDrop={(e)=>videoDropped(e,item?.id)} key={index} className='border rounded p-3 mb-2'>
      <div style={{height:'40px'}} className='d-flex justify-content-between'>
    ...
    <Modal show={show} onHide={handleClose} backdrop="static"
      keyboard={false}>
        ...
        <Modal.Footer>
          ...
          <Button onClick={handleAddCategory} className="btn btn-danger" >
            Add
          </Button>
        </Modal.Footer>
        </Modal>
